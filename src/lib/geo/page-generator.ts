...

{
  ...
  try {
    ...
    // Skip refinement if we've already used most of our time budget (Vercel 120s limit)
    const elapsedMs = Date.now() - startTime;
    const hasTimeBudget = elapsedMs < 50000; // Only refine if under 50s elapsed
    if (!options?.autoGenerated && body.length > 500 && hasTimeBudget) {
