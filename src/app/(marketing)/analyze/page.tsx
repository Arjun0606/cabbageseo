...
function FreeScoringPageContent() {
  ...
  // Generate AI-friendly export for AI coding assistants
  const handleExportForAI = async () => {
    if (!result) return;
    
    const seoFixes = result.seo.recommendations.slice(0, 10);
    const aioFixes = result.aio.recommendations.slice(0, 10);
    
    // Generate detailed fixes from all categories
    const allSeoIssues: string[] = [];
    const allAioIssues: string[] = [];
    
    // Collect SEO issues with fixes
    Object.values(result.seo.details).forEach(category => {
      category.forEach(item => {
        if (item.status !== "pass" && item.howToFix) {
          allSeoIssues.push(`- **${item.name}** (${item.score}/${item.maxScore}): ${item.reason}\n  - Fix: ${item.howToFix}`);
        }
      });
    });
    
    // Collect AIO issues with fixes
    Object.values(result.aio.details).forEach(category => {
      category.forEach(item => {
        if (item.status !== "pass" && item.howToFix) {
          allAioIssues.push(`- **${item.name}** (${item.score}/${item.maxScore}): ${item.reason}\n  - Fix: ${item.howToFix}`);
        }
      });
    });
    
    const exportContent = `# SEO & AI Visibility Analysis Report
## Website: ${result.url}
## Title: ${result.title}
...
## SCORES SUMMARY
- **SEO Score:** ${result.seoScore}/100
- **AI Visibility Score:** ${result.aioScore}/100  
- **Combined Score:** ${result.combinedScore}/100

---

## TOP PRIORITY: SEO FIXES
${seoFixes.map((fix, i) => `${i + 1}. ${fix}`).join('\n')}

---

## TOP PRIORITY: AI VISIBILITY FIXES
${aioFixes.map((fix, i) => `${i + 1}. ${fix}`).join('\n')}

---

## DETAILED SEO ISSUES (${allSeoIssues.length} found)
${allSeoIssues.join('\n\n')}

---

## DETAILED AI VISIBILITY ISSUES (${allAioIssues.length} found)
${allAioIssues.join('\n\n')}

---

## IMPLEMENTATION INSTRUCTIONS FOR AI CODING ASSISTANT

Please implement the following fixes in priority order:

### 1. Schema Markup (High Impact)
Add JSON-LD structured data to the page. Include:
- Organization schema with name, logo, URL
- FAQPage schema if there are FAQ sections
- Article/BlogPosting schema for content pages
- HowTo schema for step-by-step instructions

### 2. Meta Tags
- Ensure title is 50-60 characters with main keyword
- Meta description should be 150-160 characters with call-to-action
- Add canonical tag: \`<link rel="canonical" href="${result.url}" />\`

### 3. Content Structure for AI
- Start sections with direct definitions: "X is..." or "X refers to..."
...
- Include citations/sources for claims
    ...
    "mainEntity": [
      {
        ...
        "acceptedAnswer": { "@type": "Answer", "text": "Direct answer here..." }
      }
    ]
  }
}
...
### Quotable Content Pattern
\`\`\`html
<!-- AI loves direct definitions - use this format -->
<p><strong>[Topic] is</strong> a [clear, concise definition in one sentence].</p>

<!-- Key takeaways get cited -->
<div class="key-takeaways">
  <h3>Key Takeaways</h3>
  <ul>
    <li>✓ First key point (keep under 20 words)</li>
    <li>✓ Second key point with a specific stat or fact</li>
...
<section class="faq" itemscope itemtype="https://schema.org/FAQPage">
  ...
  <div itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
    ...
    <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
      <p itemprop="text">[Topic] is [definition]. This means [explanation].</p>
    </div>
  </div>
</section>
\`\`\`
...
<div class="author" itemscope itemtype="https://schema.org/Person">
  ...
  <span itemprop="jobTitle">SEO Expert</span> at 
  <span itemprop="worksFor">Company Name</span>
  <meta itemprop="url" content="https://linkedin.com/in/johnsmith" />
</div>
<time datetime="${new Date().toISOString().split('T')[0]}">Last updated: ${new Date().toLocaleDateString()}</time>
\`\`\`

---

## PROMPT FOR AI CODING ASSISTANT (CURSOR/CLAUDE)

Copy this entire report and paste it into your AI coding assistant with this prompt:

> "Please implement all the SEO and AI Visibility fixes in this report for my website. 
> Start with the critical issues first. Generate the actual code changes needed."

---

*Generated by CabbageSEO - https://cabbageseo.com*
*The goal: Get your content cited and recommended by AI platforms like ChatGPT, Perplexity, and Google AI.*
`;
  ...
  const handleDownloadReport = () => {
    ...
    const reportContent = `# GEO Improvement Report for Engineers
## Website: ${result.url}
## Generated: ${new Date().toISOString().split('T')[0]}

---

## GOAL: Get This Website Cited by AI

The objective is to make this content appear in:
- ChatGPT responses
...
## PRIORITY FIXES FOR ENGINEERS

### SEO Fixes (Impacts search rankings)
${seoFixes.map((fix, i) => `${i + 1}. ${fix}`).join('\n')}

### AI Visibility Fixes (Impacts AI citations)
${aioFixes.map((fix, i) => `${i + 1}. ${fix}`).join('\n')}
...
### Content Structure
- [ ] Start key paragraphs with direct definitions ("X is...")
- [ ] Add "Key Takeaways" section with bullet points
- [ ] Add FAQ section with schema markup
- [ ] Add comparison tables where relevant
- [ ] Include specific stats and citations

### Technical SEO
- [ ] Ensure meta title is 50-60 chars with primary keyword
...
### Authority Signals
- [ ] Add author name with credentials/title
- [ ] Add "Last Updated" date visible on page
- [ ] Include sources/citations for claims
...
See the full report for complete code examples including:
- JSON-LD schema templates
- Quotable content patterns
- FAQ section HTML
- Author attribution markup

---

*Generated by CabbageSEO - Get your content cited by AI*
`;
    ...
    a.href = url;
    a.download = `geo-report-${new URL(result.url).hostname}-${new Date().toISOString().split('T')[0]}.md`;
          ...
          <div className="container mx-auto max-w-6xl">
            ...
            <div className="mb-6 flex justify-between items-center">
              <div className="flex gap-2">
                ...
                >
                  {exportCopied ? <Check className="w-4 h-4 mr-2" /> : <Copy className="w-4 h-4 mr-2" />}
