...
export class AIOAuditEngine {
  ...
  fromRecommendations(recommendations: Recommendation[]): AIOIssue[] {
    return recommendations.map((rec, index) => ({
      ...
      affectedPlatforms: ["google_aio", "chatgpt", "perplexity", "claude", "gemini"],
      impact: rec.impact,
  ...
  private checkEntityDensity(result: AIOAnalysisResult): AIOIssue[] {
    ...
    if (result.entityDensity < AIO_THRESHOLDS.entityDensity.min) {
      issues.push({
        ...
        affectedPlatforms: ["google_aio", "chatgpt", "perplexity", "claude"],
        impact: "+10-15 points",
        autoFixable: true,
    ...
    if (poorlyExplainedEntities.length > result.entities.length * 0.5) {
      issues.push({
        ...
        affectedPlatforms: ["claude", "perplexity"],
        impact: "+5-10 points",
        autoFixable: true,
  ...
  private checkContentStructure(structure: ContentStructure): AIOIssue[] {
    ...
    if (!structure.hasStepByStep && structure.hasHowToSection) {
      issues.push({
        ...
        affectedPlatforms: ["google_aio", "gemini"],
        impact: "+5-10 points",
        autoFixable: true,
    ...
    if (structure.headingHierarchy === "poor") {
      issues.push({
        ...
        affectedPlatforms: ["google_aio", "claude", "gemini"],
        impact: "+5-10 points",
        autoFixable: false,
      });
    }
  ...
  private checkAuthority(structure: ContentStructure): AIOIssue[] {
    ...
    if (!structure.hasExpertAttribution) {
      issues.push({
        ...
        affectedPlatforms: ["google_aio", "perplexity", "claude"],
        impact: "+10-15 points",
        autoFixable: true,
