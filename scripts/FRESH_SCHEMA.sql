...
DROP TABLE IF EXISTS "public"."ai_citations" CASCADE;
DROP TABLE IF EXISTS "public"."aio_analyses" CASCADE;
...
-- Drop old types (ignore errors if they don't exist)
...
DROP FUNCTION IF EXISTS update_site_citation_count() CASCADE;
DROP FUNCTION IF EXISTS get_plan_limits(plan) CASCADE;
...
CREATE TABLE "public"."users" (
  ...
  "notification_settings" jsonb DEFAULT '{"citationAlerts": true, "weeklyReport": true, "competitorAlerts": true, "productUpdates": false}'::jsonb,
  "created_at" timestamptz NOT NULL DEFAULT now(),
  "updated_at" timestamptz NOT NULL DEFAULT now()
);
...
-- Prevent duplicate citations for same site/platform/query
...
-- Users can see analyses for their sites
CREATE POLICY "geo_analyses_org" ON "geo_analyses"
  FOR ALL USING (
    organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
  );
...
-- Weekly reset function (call via cron)
CREATE OR REPLACE FUNCTION reset_weekly_citations()
RETURNS void AS $$
BEGIN
  UPDATE sites
  SET citations_last_week = citations_this_week,
      citations_this_week = 0,
      updated_at = now();
  
  UPDATE competitors
  SET citations_this_week = 0,
      updated_at = now();
END;
$$ LANGUAGE plpgsql;
...
CREATE POLICY "service_role_citations" ON "citations"
  FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "service_role_competitors" ON "competitors"
  FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "service_role_usage" ON "usage"
  FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "service_role_notifications" ON "notifications"
  FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "service_role_geo_analyses" ON "geo_analyses"
  FOR ALL USING (auth.role() = 'service_role');
...
-- 
-- Tables created:
-- - organizations (billing with Dodo)
-- - users (auth)
-- - sites (tracked websites with citation stats)
-- - citations (AI mentions)
-- - competitors (competitor tracking)
-- - usage (API limits per period)
-- - notifications (alert settings)
--
-- Next: Sign up via the app to create your first user
-- ============================================
