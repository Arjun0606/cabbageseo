...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      ...
      "placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-cabbage-500 focus:ring-offset-0",
      "disabled:cursor-not-allowed disabled:opacity-50",
      "dark:border-slate-700 dark:bg-slate-900 dark:ring-offset-slate-950 dark:placeholder:text-slate-500",
      className
    )}
    {...props}
  >
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-700 shadow-lg",
  ...
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      <SelectPrimitive.ItemIndicator>
        <Check className="h-4 w-4 text-cabbage-600" />
      </SelectPrimitive.ItemIndicator>
    </span>
