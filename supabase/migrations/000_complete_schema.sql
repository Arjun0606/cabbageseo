-- ============================================
-- CabbageSEO Database Schema (COMPLETE WITH AIO)
...
-- Auto-update site AIO average when pages are analyzed
...
-- Auto-update timestamps on integrations
...
CREATE POLICY "Users can manage own org integrations" ON integrations
...
-- Sites
CREATE POLICY "Users can manage own org sites" ON sites
    FOR ALL USING (organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid()));

-- Keyword clusters
CREATE POLICY "Users can manage clusters for their sites" ON keyword_clusters
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- Keywords
CREATE POLICY "Users can manage keywords for their sites" ON keywords
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- Content
CREATE POLICY "Users can manage content for their sites" ON content
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));
...
-- Pages
CREATE POLICY "Users can manage pages for their sites" ON pages
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- Audits
CREATE POLICY "Users can manage audits for their sites" ON audits
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- Issues
CREATE POLICY "Users can manage issues for their sites" ON issues
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- Tasks
CREATE POLICY "Users can manage tasks for their org" ON tasks
    FOR ALL USING (organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid()));

-- Rankings
CREATE POLICY "Users can view rankings for their sites" ON rankings
    FOR SELECT USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- Entities
CREATE POLICY "Users can manage entities for their sites" ON entities
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- AI Citations
CREATE POLICY "Users can manage citations for their sites" ON ai_citations
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- AIO Analyses
CREATE POLICY "Users can manage AIO analyses for their sites" ON aio_analyses
    FOR ALL USING (site_id IN (
        SELECT id FROM sites WHERE organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid())
    ));

-- Usage records
CREATE POLICY "Users can view own org usage records" ON usage_records
    FOR SELECT USING (organization_id IN (SELECT organization_id FROM users WHERE id = auth.uid()));
...
-- ============================================
-- DONE! Your CabbageSEO database is ready.
-- ============================================
